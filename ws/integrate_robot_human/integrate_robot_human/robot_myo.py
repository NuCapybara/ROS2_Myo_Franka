...
class Robot_myo(Node):
    def __init__(self):
        ...
        rob_raw_data = pd.read_csv(
            "/home/jialuyu/Final_Project/data_collect_myo/ROS2_Myo_Franka/ws/models/b1k_e80k_eval/original_data_only_t.csv",
            header=None,
            skiprows=1,
        ...
        )  # Timer with 0.5s interval
        self.start = False

    def timer_callback(self):
        self.get_logger().info(
            "Timer callback executed. Delay of 0.5 seconds achieved."
        )
    ...
    def build_feed_in_emgimu(self):
        ...
        if not (
            len(self.rl_imu_buffer) >= self.imu_max_size
            and len(self.ru_imu_buffer) >= self.imu_max_size
            and len(self.rl_emg_buffer) >= self.emg_max_size
            and len(self.ru_emg_buffer) >= self.emg_max_size
        ):
            self.get_logger().info(
        ...
        rl_emg_array = np.array(self.rl_emg_buffer).reshape(
            self.emg_max_size, 8
        )  # (2884, 8)
        smoothed_rl_emg = self.smooth_and_rectify_multichannel(
            rl_emg_array, window_size=50
        )
        ru_emg_array = np.array(self.ru_emg_buffer).reshape(
            self.emg_max_size, 8
        )  # (2884, 8)
        smoothed_ru_emg = self.smooth_and_rectify_multichannel(
            ru_emg_array, window_size=50
        )

        rl_emg_array = np.array(smoothed_rl_emg).reshape(self.df_initial_size, 4, 8)
        ru_emg_array = np.array(smoothed_ru_emg).reshape(
            self.df_initial_size, 4, 8
        )  # Shape: 721 timesteps, 4 samples, 8 channels
        ...
        rl_emg_downsampled = rl_emg_array[
            :, 0, :
        ]  # downsample emg sample size to (721*8)
        ru_emg_downsampled = ru_emg_array[:, 0, :]

        if not (
            rl_emg_downsampled.shape[0] == self.df_initial_size
            and ru_emg_downsampled.shape[0] == self.df_initial_size
            and rl_imu_array.shape[0] == self.imu_max_size
            and ru_imu_array.shape[0] == self.imu_max_size
        ):
    ...
    async def check_buffers(self):
        ...
        if X_augm_test is not None:
            ...
            self.get_logger().info(
                "Buffers are ready. Proceeding with data processing."
            )
