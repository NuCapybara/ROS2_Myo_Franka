...
class pick_place(Node):
    def __init__(self):
        ...
        self.adjust_server = ActionServer(
            self,
            EmptyAction,
            "adjust",
        ...
        self.downward_orientation = Quaternion(
            x=0.9999999999991198, y=0.0, z=0.0, w=1.3267948966775328e-06
        )
    ...
    async def pick_callback(self, goal_handle):
        ...
        if result.error_code != 0:
            self.get_logger().error(
                f"Grasp execution failed with error code: {result.error_code}"
            )
        else:
    ...
    async def place_callback(self, goal_handle):
        ...
        if result.error_code != 0:
            self.get_logger().error(
                f"Place execution failed with error code: {result.error_code}"
            )
        else:
